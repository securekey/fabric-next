...
func (vr *ValidationResults) String() string {
	return fmt.Sprintf("MSP [%s], Endpoint [%s], Block: %d, TxFlags: %v, Err: [%v], Identity: %v, Signature: %v", vr.MSPID, vr.Endpoint, vr.BlockNumber, vr.TxFlags, vr.Err, vr.Identity, vr.Signature)
}
...
func New(channelID string, gossip gossipAdapter, validator validation.PolicyEvaluator, getPolicyBytes policyRetriever) *Policy {
	return &Policy{
		channelID: channelID,
		evaluator: newPolicyEvaluatorWrapper(func() (PolicyEvaluator, error) {
			...
			return newPolicyEvaluator(channelID, gossip, policyBytes, validator)
		}),
	}
}
