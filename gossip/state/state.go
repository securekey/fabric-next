...
func (s *GossipStateProviderImpl) publishBlock(block *common.Block, pvtData util.PvtDataCollections) error {
	...
	for _, validatedTxOp := range validatedTxOps {
		kvCache, _ := statedb.GetKVCache(validatedTxOp.Namespace)
		validatedTx, found := kvCache.Get(validatedTxOp.Key)
		if found && validatedTx.BlockNum < validatedTxOp.BlockNum && validatedTx.IndexInBlock < validatedTxOp.IndexInBlock && validatedTxOp.IsDeleted {
			kvCache.Remove(validatedTxOp.ValidatedTx.Key)
		} else if found && validatedTx.BlockNum < validatedTxOp.BlockNum && validatedTx.IndexInBlock < validatedTxOp.IndexInBlock || !found {
