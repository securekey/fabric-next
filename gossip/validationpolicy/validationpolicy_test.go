...
func TestEvaluatePeerGroups(t *testing.T) {
	...
	t.Run("Current Org", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			getPolicyBytes(fmt.Sprintf("AND ('%s.member')", org1MSPID)),
			mocks.NewPolicyEvaluator(),
		)
		require.NoError(t, err)
		evaluator.policy.MaxGroups = 0
	...
	t.Run("1 Of 3", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			getPolicyBytes(fmt.Sprintf("OR ('%s.member','%s.member','%s.member')", org1MSPID, org2MSPID, org3MSPID)),
			mocks.NewPolicyEvaluator(),
		)
		require.NoError(t, err)

		maxGroups := 3
		evaluator.policy.MaxGroups = maxGroups
	...
	t.Run("2 Of 3", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			getPolicyBytes(fmt.Sprintf("OutOf(2, '%s.member','%s.member','%s.member')", org1MSPID, org2MSPID, org3MSPID)),
			mocks.NewPolicyEvaluator(),
		)
		require.NoError(t, err)
	...
	t.Run("3 Of 3", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			getPolicyBytes(fmt.Sprintf("AND('%s.member','%s.member','%s.member')", org1MSPID, org2MSPID, org3MSPID)),
			mocks.NewPolicyEvaluator(),
		)
		require.NoError(t, err)
...
func TestValidationPolicy(t *testing.T) {
	...
	t.Run("Error Retrieving Policy", func(t *testing.T) {
		policy := New("testchannel", gossip, mocks.NewPolicyEvaluator(),
