...
func NewGossipStateProvider(chainID string, services *ServicesMediator, ledger ledgerResources, isCommitter bool, committer committer.Committer) GossipStateProvider {
...
func (s *GossipStateProviderImpl) getBlockFromLedger(number uint64) (*common.Block, error) {
	...
	for i := 0; i < attempts; i++ {
		blocks = s.committer.GetBlocks([]uint64{number})
		if len(blocks) > 0 {
			// FIXME: Change to Debugf
	...
	if len(blocks) == 0 {
		return nil, errors.Errorf("Unable to get block number %d from ledger", number)
	}

	if blocks[0].Header.Number != number {
		return nil, errors.Errorf("Expecting block number %d from ledger but got %d", number, blocks[0].Header.Number)
	}
