...
import (
	...
	"github.com/hyperledger/fabric/integration/mocks"
...
func TestCanReturnProvider(t *testing.T) {
	...
	defer cleanup()
	provider, err := historycouchdb.NewHistoryDBProvider(def)
...
func TestProviderCanGetDbHandle(t *testing.T) {
	def, cleanup := startCouchDB()
	defer cleanup()
	provider, _ := historycouchdb.NewHistoryDBProvider(def)
...
func queryCouchDbById(def *couchdb.CouchDBDef, dbname, id string) (*[]couchdb.QueryResult, error) {
	return newCouchDbClient(
		def,
		couchdb.ConstructBlockchainDBName(dbname, historycouchdb.DbNameSuffix),
	).QueryDocuments(
