...
func (c *cachedStateStore) AcceptPreloadedCommittedVersions(preoptimized map[*statedb.CompositeKey]*version.Height) {
	c.bulkOptimizable.AcceptPreloadedCommittedVersions(preoptimized)
}
func (c *cachedStateStore) LoadCommittedVersions(keys []*statedb.CompositeKey) error {
	...
	for _, key := range keys {
		metadata, found, err := stateidx.GetMetadata(&statekeyindex.CompositeKey{Key: key.Key, Namespace: key.Namespace})
		if err != nil {
	...
	c.bulkOptimizable.AcceptPreloadedCommittedVersions(preloaded)
	return nil
}
