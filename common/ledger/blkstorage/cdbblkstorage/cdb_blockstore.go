...
func newCDBBlockStore(db *couchdb.CouchDatabase, ledgerID string, indexConfig *blkstorage.IndexConfig) *cdbBlockStore {
	...
	cpInfo, err := cdbBlockStore.loadCurrentInfo()
	if err != nil {
	...
	err = cdbBlockStore.saveCurrentInfo(cpInfo)
	if err != nil {
...
func (s *cdbBlockStore) AddBlock(block *common.Block) error {
	...
	if err = s.saveCurrentInfo(newCPInfo); err != nil {
...
func (s *cdbBlockStore) loadCurrentInfo() (*checkpointInfo, error) {
	doc, _, err := s.db.ReadDoc(blkMgrInfoKey)
	if err != nil {
