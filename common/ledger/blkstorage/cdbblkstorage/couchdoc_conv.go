...
const (
	...
	blockTxnIDsField     = "transactionIDs"
	blockTxnIDField      = "id"
...
func retrieveTxValidationCodeQuery(db *couchdb.CouchDatabase, query string) (int64, error) {
	...
	jsonMap, err := jsonValueToJSON(results[0].Value)
	if err != nil {
		return 0, errors.Wrapf(err, "couchDocToJSON return error")
	}

	txnValidationCodeUT, ok := jsonMap[txnValidationCode]
	if !ok {
		return 0, errors.New("validation code not found in couch document")
	}

	txnValidationCodeStored, ok := txnValidationCodeUT.(string)
	if !ok {
		return 0, errors.New("unexpected type in couch document")
	}
	...
	if err != nil {
		return 0, errors.Wrapf(err, "validation code was invalid for transaction ID [%s]", txnValidationCodeStored)
	}
