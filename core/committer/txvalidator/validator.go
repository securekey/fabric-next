...
func (v *TxValidator) gossipValidationResults(block *common.Block, txFlags ledgerUtil.TxValidationFlags) {
	...
	if err != nil {
		logger.Errorf("[%s] Unable to determine whether the validation policy is local-org: %s", v.ChainID, err)
		return
	}

	// If using single-org policy then only gossip to the committer within the org
	var committers []*roleutil.Member
	if isLocalPolicy {
		if ledgerconfig.IsCommitter() {
			// FIXME: Change to Debug
			logger.Infof("[%s] Using 'local-org' policy. Will not gossip the validation results since I am the committing peer for the local org...", v.ChainID)
			return
		}

		// FIXME: Change to Debug
		logger.Infof("[%s] Using 'local-org' policy. Will only gossip the validation results to the committing peer for the local org...", v.ChainID)
		committer, err := v.roleUtil.LocalOrgCommitter(false)
		if err != nil {
			logger.Errorf("[%s] Unable to get the committing peer for the local org to send the validation response to: %s", v.ChainID, err)
			return
		}
		committers = []*roleutil.Member{committer}
	} else if committers, err = v.roleUtil.Committers(false); err != nil {
