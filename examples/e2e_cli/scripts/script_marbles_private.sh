...
installChaincode () {
	PEER=$1
	setGlobals $PEER
	peer chaincode install -n marblesp -v 1.0 -p github.com/hyperledger/fabric/examples/chaincode/go/marbles02_private >&log.txt
	res=$?
	cat log.txt
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "OR	('Org1MSP.member','Org2MSP.member')" --collections-config collections.json >&log.txt
	else
		peer chaincode instantiate -o orderer.example.com:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n mycc -v 1.0 -c '{"Args":["init","a","100","b","200"]}' -P "OR	('Org1MSP.member','Org2MSP.member')" --collections-config collections.json >&log.txt
	fi
	res=$?
	cat log.txt
...
echo
echo "===================== All GOOD, End-2-End setup for marbles02_private ===================== "
echo
