...
func (vdb *VersionedDB) LoadWSetCommittedVersions(keys []*statedb.CompositeKey, preLoaded map[*statedb.CompositeKey]*version.Height) error {
...
func (vdb *VersionedDB) ClearCachedVersions() {
	...
	vdb.verWSetCacheLock.Lock()
	vdb.committedWSetDataCache = newVersionCache()
	vdb.verWSetCacheLock.Unlock()
}
