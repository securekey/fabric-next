...
func (s *store) PersistWithConfig(txid string, blockHeight uint64, privateSimulationResultsWithConfig *pb.TxPvtReadWriteSetWithConfigInfo) error {
	if metrics.IsDebug() {
		// Measure the whole
		stopWatch := metrics.RootScope.Timer("memtransientdata_persistwithconfig_duration").Start()
...
func (s *store) GetTxPvtRWSetByTxid(txid string, filter ledger.PvtNsCollFilter, endorsers []*peer.Endorsement) (transientstore.RWSetScanner, error) {
	if metrics.IsDebug() {
		// Measure the whole
		stopWatch := metrics.RootScope.Timer("memtransientdata_gettxpvtrwsetbytxid_duration").Start()
		defer stopWatch.Stop()
	}
...
func (s *store) PurgeByTxids(txids []string) error {
	if metrics.IsDebug() {
		// Measure the whole
		stopWatch := metrics.RootScope.Timer("memtransientdata_purgebytxids_duration").Start()
		defer stopWatch.Stop()
	}
