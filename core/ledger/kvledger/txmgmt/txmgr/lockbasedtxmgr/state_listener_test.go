...
func TestStateListener(t *testing.T) {
	...
	tx := &current{block: dummyBlock, batch: sampleBatch}
	txmgr.invokeNamespaceListeners(tx)
	txmgr.commitPipeline <- tx
	...
	txmgr.reset()
	tx = &current{block: common.NewBlock(2, []byte("anotherDummyHash")), batch: sampleBatch}
	txmgr.invokeNamespaceListeners(tx)
	txmgr.commitPipeline <- tx
	assert.Equal(t, 1, ml1.HandleStateUpdatesCallCount())
	assert.Equal(t, 1, ml2.HandleStateUpdatesCallCount())
