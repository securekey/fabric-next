...
func (vdb *VersionedDB) retrieveMetadata(nsKeysMap map[string][]string, includeDoc bool) (map[string][]*couchdb.DocMetadata, error) {
	...
	for ns, keys := range nsKeysMap {
		...
		nsMetadataRetrievers = append(nsMetadataRetrievers, &nsMetadataRetriever{ns: ns, db: db, keys: keys, includeDoc: includeDoc})
	}
...
func retrieveNsMetadata(db *couchdb.CouchDatabase, keys []string, includeDoc bool) ([]*couchdb.DocMetadata, error) {
	...
	for {
		...
		batch := &subNsMetadataRetriever{db: db, keys: remainingKeys[:numKeys], includeDoc: includeDoc}
...
func (r *nsMetadataRetriever) execute() error {
	var err error
	if r.executionResult, err = retrieveNsMetadata(r.db, r.keys, r.includeDoc); err != nil {
		return err
	}
	return nil
}
...
func (b *subNsMetadataRetriever) execute() error {
	var err error
	if b.executionResult, err = b.db.BatchRetrieveDocumentMetadata(b.keys, b.includeDoc); err != nil {
		return err
	}
	return nil
}
