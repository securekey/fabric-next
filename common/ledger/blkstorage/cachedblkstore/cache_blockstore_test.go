...
func TestRetrieveTxByBlockNumTranNum(t *testing.T) {
	...
	eb0 := createMockBlockWithTxn(0, &mockTx{"a", "keya", peer.TxValidationCode_VALID})
	eb1 := createMockBlockWithTxn(1, &mockTx{"b", "keyb", peer.TxValidationCode_VALID})
	eb2 := createMockBlockWithTxn(2, &mockTx{"c", "keyc", peer.TxValidationCode_VALID})
	cbs.blockStore.(*mockBlockStoreWithCheckpoint).BlocksByNumber[0] = eb0
	cbs.blockStore.(*mockBlockStoreWithCheckpoint).BlocksByNumber[1] = eb1
...
func TestRetrieveTxValidationCodeByTxID(t *testing.T) {
	...
	eb0 := createMockBlockWithTxn(0, &mockTx{"a", "keya", etx0})
	eb1 := createMockBlockWithTxn(1, &mockTx{"b", "keyb", etx1})
	eb2 := createMockBlockWithTxn(2, &mockTx{"c", "keyc", etx2})
	cbs.blockStore.(*mockBlockStoreWithCheckpoint).BlocksByNumber[0] = eb0
	cbs.blockStore.(*mockBlockStoreWithCheckpoint).BlocksByNumber[1] = eb1
...
func createMockBlockWithTxn(blockNum uint64, tx *mockTx) *common.Block {
	...
	b := mocks.NewBlock(
		...
		[]peer.TxValidationCode{tx.ValidationCode},
		mocks.NewTransaction(
			&rwsetutil.NsRwSet{
				NameSpace: namespace,
				KvRwSet: &kvrwset.KVRWSet{
					Writes: []*kvrwset.KVWrite{{Key: tx.Key, IsDelete: false, Value: []byte("some_value")}},
				},
			},
		),
	)
...
func createMockBlock(blockNum uint64) *common.Block {
	...
	return createMockBlockWithTxn(blockNum, &mockTx{txID, key, peer.TxValidationCode_VALID})
}
