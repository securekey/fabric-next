...
func (s *GossipStateProviderImpl) publishBlock(block *common.Block) error {
	...
	kvMap, err := getKVFromBlock(committedBlock)
	if err != nil {
		return err
	}
	for key, value := range kvMap {
		fmt.Printf("*** kvMap key %s value %s\n", key, value)
	}

	return nil
}

func getKVFromBlock(block *common.Block) (map[string][]byte, error) {
		...
		if txRWSet != nil {
			for _, nsRwSet := range txRWSet.NsRwSets {
				...
				for _, kvwrite := range pubWriteset.Writes {
					kvMap[fmt.Sprintf("%s!%s", nsRwSet.NameSpace, kvwrite.Key)] = kvwrite.Value
				}
			}
		}
	}
