...
func (dbclient *CouchDatabase) BatchRetrieveDocument(keys []string) ([]*NamedCouchDoc, error) {
	...
	var jsonValueResponse BatchRetreiveDocValueResponse
	err = json.Unmarshal(jsonResponseRaw, &jsonValueResponse)
	if err != nil {
		return nil, err
	}
	...
	for i, row := range jsonResponse.Rows {
		...
		doc := CouchDoc{JSONValue:jsonValueResponse.Rows[i], Attachments: attachments}
