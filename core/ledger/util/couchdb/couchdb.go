...
func (couchInstance *CouchInstance) VerifyCouchConfig() (*ConnectionInfo, error) {
	...
	if err != nil {
		logger.Errorf("URL parse error: %s", err)
		return nil, errors.Wrapf(err, "error parsing couch instance URL: %s", couchInstance.conf.URL)
	}
	connectURL.Path = "/"
	...
	if err != nil {
		...
		return nil,  errors.WithMessage(err, "unable to connect to CouchDB. Check the admin username and password")
	}
	return dbResponse,  nil
}
...
func (dbclient *CouchDatabase) GetDatabaseSecurity() (*DatabaseSecurity, error) {
	...
	//get the number of retries
	maxRetries := dbclient.CouchInstance.conf.MaxRetries
	resp, err := dbclient.CouchInstance.handleRequest(http.MethodGet, securityURL.String(), nil, "", "", maxRetries, true)
	if err != nil {
		return nil, err
	}
	defer closeResponseBody(resp)
