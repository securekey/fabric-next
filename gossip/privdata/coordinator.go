...
func (c *coordinator) ValidateBlock(block *common.Block, privateDataSets util.PvtDataCollections, resultsChan chan *txvalidator.ValidationResults) (*ledger.BlockAndPvtData, []string, error) {
	...
	logger.Debugf("[%s] ... finished first phase validation of block %d against committed data in %s. Starting second phase...", c.ChainID, block.Header.Number, time.Since(begin))
	...
	logger.Debugf("[%s] ... finished second phase validation of block %d and private data in %s", c.ChainID, blockAndPvtData.Block.Header.Number, time.Since(begin))
