...
func GetFromKVCache(chId string, namespace string, key string) (*VersionedValue, bool) {
	...
	logger.Infof("Looking for key[%s] in the cache chId[%s], namespace[%s]", key, chId, namespace)
	...
	logger.Errorf("Failed to find key[%s] in the cache chId[%s], namespace[%s]", key, chId, namespace)
