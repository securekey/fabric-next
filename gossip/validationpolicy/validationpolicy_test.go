...
func TestValidationPolicy(t *testing.T) {
	...
	t.Run("Org Policy", func(t *testing.T) {
		...
		policy := New("testchannel", gossip, mocks.NewPolicyProvider(),
			func() ([]byte, error) {
				return getPolicyBytes(fmt.Sprintf("MAX(%d,AND('%s.member'))", maxGroups, org1MSPID)), nil
			},
		)
		require.NotNil(t, policy)
	...
	t.Run("Validate Results", func(t *testing.T) {
		policy := New("testchannel", gossip, mocks.NewPolicyProvider(),
			func() ([]byte, error) {
				return getPolicyBytes(fmt.Sprintf("MAX(3,AND('%s.member'))", org1MSPID)), nil
			},
		)
		require.NotNil(t, policy)
