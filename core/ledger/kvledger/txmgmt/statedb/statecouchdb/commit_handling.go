...
func (committer *subNsCommitter) execute() error {
	...
	case <-committer.startCommitting:
		logger.Info("committer received order to begin committing...")
...
// Forwards the notification that the statekeyindex is ready to the given committers.
func (vdb *VersionedDB) letThemKnowWhenStateKeyIndexIsReady(committers []*subNsCommitter) {
	select{
		case <-vdb.stateKeyIndexReadyCh:
			logger.Infof("received notification that statekeyindex is ready. Forwarding to %d committers.", len(committers))
			for _, committer := range committers {
				committer.startCommitting <- struct{}{}
			}
	}
}
