...
func lookupMetadata(db *couchdb.CouchDatabase) (metadata, bool, error) {
	...
	for i := 1; i <= mc; i++ {
		...
		if doc != nil {
			...
			dk := decodeDatakey(lastPvtDataResp.Data[dataField])
			lastBlockNum = dk.blkNum
			break
		}
	}

	if lastPvtDataResp == nil || lastBlockNum <= 0 {
		return metadata{pending: false}, false, nil
	}

	m := metadata{true, lastBlockNum}
	return m, true, nil
}
