...
func CreateMockBlock(blockNum uint64, mockTxns ...*Transaction) *common.Block {
	var envelopes []*common.Envelope
	var validationCodes []peer.TxValidationCode

	for _, mockTxn := range mockTxns {
		envelopes = append(envelopes, CreateEnvelope(mockTxn.ChannelHeader, mockTxn.Txn))
		validationCodes = append(validationCodes, mockTxn.ValidationCode)
	}
...
func CreateSimpleMockBlock(blockNum uint64) *common.Block {
	...
	return CreateMockBlock(blockNum, NewSimpleTransaction(txID, key, peer.TxValidationCode_VALID))
}
...
func NewSimpleTransaction(txID string, key string, validationCode peer.TxValidationCode) *Transaction {
