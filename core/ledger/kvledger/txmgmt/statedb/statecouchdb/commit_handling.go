...
func (committer *subNsCommitter) execute() error {
	...
	for _, updateDocument := range committer.batchUpdateMap {
		...
		metadata, found, err := committer.statekeyindex.GetMetadata(&statekeyindex.CompositeKey{Key: keyval.key, Namespace: committer.namespace})
		if found {
	...
	for _, respDoc := range batchUpdateResp {
		if respDoc.Ok {
			keyMetadata[respDoc.ID].CouchDbRev = respDoc.Rev
		} else {
			...
			if batchUpdateDocument.Deleted {
				...
				keyMetadata[respDoc.ID].CouchDbRev = rev
			}
