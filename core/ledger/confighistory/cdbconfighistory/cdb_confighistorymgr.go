...
func (r *retriever) mostRecentEntryBelow(blockNum uint64, ns, key string) (*compositeKV, error) {
	logger.Errorf("mostRecentEntryBelow() - {%s, %s, %d}", ns, key, blockNum)
	if blockNum == 0 {
	...
	for num := blockNum - 1; num > 0; num-- {
		...
		logger.Errorf("mostRecentEntryBelow() - get key %s", k1)
		doc, _, err := r.db.ReadDoc(k1)
		if err != nil {
			return nil, err
		}
		if doc == nil {
			logger.Errorf("mostRecentEntryBelow() - Doc is nil for key %s - trying next key", k1)
			continue
		}
		logger.Errorf("mostRecentEntryBelow() - Found config doc for key %s in block %d: %+v", key, num, doc)
		value, err := couchAttachmentsValue(doc.Attachments)
		if err != nil {
			logger.Errorf("mostRecentEntryBelow() - Error getting couchAttachmentsValue: %s", err)
			return nil, err
		}
		logger.Errorf("mostRecentEntryBelow() - Got couchAttachmentsValue for key %s: %s", key, value)
		k := decodeCompositeKey(compositeKey)
		logger.Errorf("mostRecentEntryBelow() - Returning %s=%s", k, value)
		return &compositeKV{k, value}, nil
	}
	logger.Errorf("mostRecentEntryBelow() - Doc is nil for block %d", blockNum-1)
	return nil, nil
}

func (r *retriever) entryAt(blockNum uint64, ns, key string) (*compositeKV, error) {
	logger.Errorf("entryAt() - {%s, %s, %d}", ns, key, blockNum)
