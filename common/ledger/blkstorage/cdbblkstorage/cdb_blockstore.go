...
func newCDBBlockStore(blockStore *couchdb.CouchDatabase, ledgerID string, createIndex bool) *cdbBlockStore {
...
func (s *cdbBlockStore) RetrieveBlockByTxID(txID string) (*common.Block, error) {
	...
	block, err := retrieveBlockQuery(s.blockStore, fmt.Sprintf(queryFmt, txID, addBlkTxnIndex))
	if err != nil {
		// note: allow ErrNotFoundInIndex to pass through
		return nil, err
	}
