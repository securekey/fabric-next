	...
	for {
		...
		case data := <-v.preLoadCommittedVersionOfWSetCh:
			...
			for i, tx := range data.block.Txs {
				...
				logger.Infof("Pre-loading write sets for Tx index %d in block %d", i, data.block.Num)
				for _, nsRWSet := range tx.RWSet.NsRwSets {
...
func (v *Validator) ValidateMVCC(block *valinternal.Block, txsFilter util.TxValidationFlags, doMVCCValidation bool, acceptTx util.TxFilter) error {
	...
	if v.db.IsBulkOptimizable() {
		...
		// FIXME: Set private data for committers
		v.preLoadCommittedVersionOfWSetCh <- &blockAndPvtData{block, nil, acceptTx}
	}
	...
	go func() {
		for _, tx := range txs {
			...
			go func(tx *valinternal.Transaction) {
				...
				validationCode, err := v.validateEndorserTXPhase1(tx.RWSet, doMVCCValidation)
				if err != nil {
...
func (v *Validator) validateEndorserTXPhase1(
	txRWSet *rwsetutil.TxRwSet,
	...
	//mvccvalidation, may invalidate transaction
	if doMVCCValidation {
		validationCode, err = v.validateTxMVCC(txRWSet)
	}
	return validationCode, err
}
