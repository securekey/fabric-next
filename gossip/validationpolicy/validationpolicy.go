...
type PolicyRetriever func() (policy, orgPolicy []byte, err error)
...
func New(channelID string, gossip gossipAdapter, pp policies.Provider, getPolicyBytes PolicyRetriever) *Policy {
	return &Policy{
		...
		evaluator: newPolicyEvaluatorWrapper(func() (policyEvaluator, error) {
			policyBytes, orgPolicyBytes, err := getPolicyBytes()
			if err != nil {
			...
			if bytes.Equal(policyBytes, orgPolicyBytes) {
				validator = orgValidator
			} else {
				validator, _, err = pp.NewPolicy(policyBytes)
				if err != nil {
					logger.Warningf("[%s] Error creating policy evaluator: %s", channelID, err)
					return nil, err
				}
			...
			policy, err := newInquireablePolicy(channelID, policyBytes)
			if err != nil {
				return nil, err
			}
			return newEvaluator(channelID, gossip, validator, orgValidator, policy)
		}),
	}
}
