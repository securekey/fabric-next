...
func (c *ExtSCCClient) invoke(function string, args [][]byte) error {
	...
	channelId := ""
	fmt.Printf("===============>args[1] from client is %#v\n", args[1])
	if args[1] != nil && len(args[1]) > 0 {
		channelId = fmt.Sprintf("%s", args[1])
		fmt.Printf("===============>setting channel to %s\n", channelId)
	}
	fmt.Printf("===============>args[2] from client is %#v\n", args[2])
	extSCCN := extSCC
	isBin, err := strconv.ParseBool(string(args[2]))
	//get bin EXT SCC if bin is set
	if err == nil && isBin {
		extSCCN = extSCCBin
		fmt.Printf("===============>calling EXTSCC caller: %s\n", extSCCN)
	}
	...
	fmt.Printf("invoking %s with args:", extSCCN)
	for p := range funcAndArgs {
		fmt.Printf("\t %s: %#v\n", p, fmt.Sprintf("%s", funcAndArgs[p]))
	}
	...
	if err != nil {
		return fmt.Errorf("Error endorsing: %s", err)
	}

	if proposalResp.Response.Status != ok {
		return fmt.Errorf("Error returned from endorser - Status: %d, Message: %s", proposalResp.Response.Status, proposalResp.Response.Message)
	}
	...
	fmt.Printf("Successfully invoked %s. Payload recieved: %s", extSCC, proposalResp.Response.Payload)
	return nil
}
