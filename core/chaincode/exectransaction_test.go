...
func deploy(ctx context.Context, cccid *ccprovider.CCContext, spec *pb.ChaincodeSpec, blockNumber uint64) (b []byte, err error) {
	...
	return deploy2(ctx, cccid, cdDeploymentSpec, nil, blockNumber)
}

func deployWithCollectionConfigs(ctx context.Context, cccid *ccprovider.CCContext, spec *pb.ChaincodeSpec,
	collectionConfigPkg *common.CollectionConfigPackage, blockNumber uint64) (b []byte, err error) {
	// First build and get the deployment spec
	cdDeploymentSpec, err := getDeploymentSpec(ctx, spec)
	if err != nil {
		return nil, err
	}
	return deploy2(ctx, cccid, cdDeploymentSpec, collectionConfigPkg, blockNumber)
}
