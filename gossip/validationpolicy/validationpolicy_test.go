...
func TestEvaluatePeerGroups(t *testing.T) {
	...
	t.Run("Current Org", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			mocks.NewPolicyEvaluator(),
			newPolicy(fmt.Sprintf("AND ('%s.member')", org1MSPID), 0),
		)
	...
	t.Run("1 Of 3", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			mocks.NewPolicyEvaluator(),
			newPolicy(fmt.Sprintf("OR ('%s.member','%s.member','%s.member')", org1MSPID, org2MSPID, org3MSPID), maxGroups),
		)
		require.NoError(t, err)
	...
	t.Run("2 Of 3", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			mocks.NewPolicyEvaluator(),
			newPolicy(fmt.Sprintf("OutOf(2, '%s.member','%s.member','%s.member')", org1MSPID, org2MSPID, org3MSPID), maxGroups),
		)
		require.NoError(t, err)
	...
	t.Run("3 Of 3", func(t *testing.T) {
		...
		evaluator, err := newPolicyEvaluator(
			"testchannel", gossip,
			mocks.NewPolicyEvaluator(),
			newPolicy(fmt.Sprintf("AND('%s.member','%s.member','%s.member')", org1MSPID, org2MSPID, org3MSPID), maxGroups),
		)
		require.NoError(t, err)
...
func TestValidationPolicy(t *testing.T) {
	...
	t.Run("Org Policy", func(t *testing.T) {
		policy := New("testchannel", gossip, mocks.NewPolicyProvider(),
			func() ([]byte, error) {
				return getPolicyBytes(fmt.Sprintf("AND ('%s.member')", org1MSPID)), nil
			},
		)
		require.NotNil(t, policy)
		...
		require.NoError(t, err)
		require.Equal(t, len(g1Peers), len(peers))
	})
...
func newPolicy(expression string, maxGroups int) *InquireableValidationPolicy {
	policy, err := newInquireablePolicy(getPolicyBytes(expression), maxGroups)
	if err != nil {
		panic(err.Error())
	}
	return policy
