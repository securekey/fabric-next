...
func (index *blockIndex) indexBlock(blockIdxInfo *blockIdxInfo) error {
	...
	logger.Errorf("Indexing block [%s]", blockIdxInfo)
...
func (index *blockIndex) GetTxLoc(txID string) (blkstorage.TxBlockLoc, error) {
	if _, ok := index.indexItemsMap[blkstorage.IndexableAttrTxID]; !ok {
	...
	txFLP := &blockLocPointer{}
	txFLP.unmarshal(b)
	return txFLP, nil
}

/*
// The current implementation of GetTxLoc contains the block number.
func (index *blockIndex) GetBlockNumberByTxID(txID string) (uint64, error) {
	if _, ok := index.indexItemsMap[blkstorage.IndexableAttrBlockTxID]; !ok {
...
func (index *blockIndex) GetTxLocByBlockNumTranNum(blockNum uint64, tranNum uint64) (blkstorage.TxBlockLoc, error) {
	if _, ok := index.indexItemsMap[blkstorage.IndexableAttrBlockNumTranNum]; !ok {
		return nil, blkstorage.ErrAttrNotIndexed
	}
...
func (lp *blockLocPointer) marshal() ([]byte, error) {
	...
	err = buffer.EncodeVarint(uint64(lp.txnNumber))
	if err != nil {
		return nil, err
	}
